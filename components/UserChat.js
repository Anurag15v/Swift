...
const UserChat = ({ item }) => {
  ...
  const fetchLastReadMessages = async () => {
    try {
      const res = await fetch(`http://192.168.152.216:8000/last-message/${userId}/${item._id}`);
      if (res.ok) {
  ...
  const fetchLastUnreadMessages = async () => {
    try {
      const res = await fetch(`http://192.168.152.216:8000/unread-last-message/${userId}/${item._id}`);
      if (res.ok) {
        const data = await res.json();
        if (data.messages)
          setMessage(data.messages);
  ...
  const fetchUnreadMessagesCount = async () => {
    try {
      const res = await fetch(`http://192.168.152.216:8000/unread-message-count/${userId}/${item._id}`);
      if (res.ok) {
        const data = await res.json();
        if (data.message) {
          console.log(data)
